...
describe('titleCaseFieldLabels', () => {
  test('title-cases scatter axis names set from column names', () => {
    let out = enrich(
      {color: ['#000'], xAxis: {name: 'sales_usd0k'}, yAxis: {name: 'profit_margin'}, series: [{type: 'scatter', name: 'profit_margin', encode: {x: 'sales_usd0k', y: 'profit_margin'}}]} as any,
      rows,
      fields,
    )
  ...
  test('leaves splitBy-generated series names (raw data values) untouched', () => {
    let out = enrich(
      {color: ['#000'], xAxis: {name: 'sales_usd0k'}, yAxis: {}, series: [{type: 'scatter', encode: {x: 'sales_usd0k', y: 'order_count', splitBy: 'customer_segment'}}]} as any,
      rows,
      fields,
    )
    expect(out.xAxis[0].name).toBe('Sales Usd0k')
