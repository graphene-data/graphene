...
export async function captureScreenshot(markdown: string, repoId: string): Promise<RenderResult> {
...
function toModelOutput({output}: {output: {success: boolean; mdId?: string; screenshot?: string; queryData?: Record<string, {rows: any[]}>; errors?: {message: string; id?: string}[]; error?: string}}) {
