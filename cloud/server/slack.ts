...
export async function slackStatus(req: FastifyRequest, reply: FastifyReply) {
  ...
  let installation = await getDb()
    .select({teamId: slackInstallations.teamId, teamName: slackInstallations.teamName})
...
async function handleAppMention(install: SlackInstallation, mention: AppMentionEvent) {
  ...
  await slackApi('reactions.add', {channel: mention.channel, timestamp: mention.ts, name: 'thought_balloon'}, install).catch(e => console.warn(e)) // errors here aren't fatal, but interesting
  ...
  try {
    ...
    await db.update(agentSessions).set({lastSlackThreadTs: mention.ts, updatedAt: new Date()}).where(eq(agentSessions.id, session.id))
  } finally {
...
async function findOrCreateSlackSession({orgId, repoId, channel, threadTs}: {orgId: string; repoId: string; channel: string; threadTs: string}) {
  ...
  let existing = await db
    .select()
    .from(agentSessions)
    .where(and(eq(agentSessions.orgId, orgId), eq(agentSessions.slackChannel, channel), eq(agentSessions.slackThreadTs, threadTs)))
    .then(rows => rows[0])
...
async function replaceUserIdsWithNames(messages: string[], installation: SlackInstallation) {
  ...
  await Promise.allSettled(
    Object.keys(map).map(async userId => {
  ...
  return messages.map(message =>
    message.replace(mentionRegex, (_, userId: string) => {
