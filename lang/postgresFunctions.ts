...
export const postgresFunctions: FunctionDef[] = [
  ...
  fn({name: 'first_value', args: [{name: 'expr', type: 'T'}], returns: 'T', window: true, metadata: 'idempotent', url: `${pg}-window.html`}),
  fn({name: 'last_value', args: [{name: 'expr', type: 'T'}], returns: 'T', window: true, metadata: 'idempotent', url: `${pg}-window.html`}),
  fn({
    name: 'nth_value',
    args: [
      {name: 'expr', type: 'T'},
  ...
  aggregate('avg', [{name: 'arg', type: 'number'}], 'number', {metadata: 'idempotent'}),
  aggregate('bit_and', [{name: 'arg', type: 'number'}], 'number'),
  aggregate('bit_or', [{name: 'arg', type: 'number'}], 'number'),
  aggregate('bool_and', [{name: 'arg', type: 'boolean'}], 'boolean', {fanoutSafe: true, aliases: ['every'], metadata: 'idempotent'}),
  aggregate('bool_or', [{name: 'arg', type: 'boolean'}], 'boolean', {fanoutSafe: true, metadata: 'idempotent'}),
  aggregate('count', [{name: 'arg', type: 'any?'}], 'number', {metadata: 'counting'}),
  aggregate('count_if', [{name: 'condition', type: 'boolean'}], 'number', {sqlTemplate: 'COUNT(*) FILTER (WHERE ${condition})', metadata: 'counting'}),
  aggregate('max', [{name: 'arg', type: 'T'}], 'T', {fanoutSafe: true, metadata: 'idempotent'}),
  aggregate('min', [{name: 'arg', type: 'T'}], 'T', {fanoutSafe: true, metadata: 'idempotent'}),
  aggregate('sum', [{name: 'arg', type: 'number'}], 'number', {metadata: 'additive'}),
  ...
  ...['abs', 'ceil', 'ceiling', 'floor', 'round', 'trunc'].map(name => scalar(name, [{name: 'x', type: 'number'}], 'number', {metadata: 'idempotent'})),
  ...['acos', 'acosh', 'asin', 'asinh', 'atan', 'atanh', 'cbrt', 'cos', 'cot', 'degrees', 'exp', 'ln', 'log', 'radians', 'sign', 'sin', 'sqrt', 'tan'].map(name =>
    scalar(name, [{name: 'x', type: 'number'}], 'number'),
  ),
  scalar(
    'atan2',
    [
