...
export function lookup(node, query, scope) {
    let pathSegments = node.getChildren('Identifier').map(i => txt(i));
    if (pathSegments.length == 0)
        throw new Error('tried to lookup empty ColumnRef');
    let colName = pathSegments.pop();
    let searchAllTablesInQuery = (name) => {
        ...
        if (matches.length > 1)
            throw new Error(`Ambiguous reference to ${name}`);
        return matches[0];
    };
    ...
    for (let part of pathSegments) {
        if (scope) { // our lookup is constrained to a single table (ie, you're in the middle of a dot-join or a measure)
            if (!scope.tableName)
                throw new Error('Missing tableName in scope');
            let field = TABLE_MAP[scope.tableName].fields[part];
            if (field.type != 'join')
                throw new Error("Trying to join on a column that isn't a join.");
    ...
    if (scope) {
        ...
        if (!tablish.fields[colName])
            throw new Error(`Couldn't find column ${colName} in ${scope.alias}`);
