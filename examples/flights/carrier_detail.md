...
<Dropdown name=carrier data=carriers_list value=code label=nickname title="Carrier" defaultValue="WN" />

```sql carrier_info
from carriers
where code = $carrier
select nickname, code
```

```sql carrier_ranks
from (from flights select carriers.code as code, count() as total_flights, on_time_arrival_rate)
select code, rank() over (order by total_flights desc) as flights_rank, rank() over (order by on_time_arrival_rate desc) as ontime_rank
```

```sql kpis
from flights
where carriers.code = $carrier
select
...
**<Value data=carrier_info column=nickname /> (<Value data=carrier_info column=code />)** has been operating since <Value data=kpis column=operating_since />, with <Value data=kpis column=total_flights fmt=num0 /> flights to <Value data=kpis column=destinations /> destinations.
...
```sql delay_dist
from flights
where carriers.code = $carrier and cancelled = 'N'
select
  case
