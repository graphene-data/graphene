...
export async function runMdFile(options: RunMdFileOptions): Promise<boolean> {
  ...
  let request = {mdFile, action: 'check' as const, chart: options.chart, inputs: options.inputs, log}
...
async function runHeadlessPageRequest({mdFile, action, chart, inputs, log}: {mdFile: string; action: 'check' | 'list'; chart?: string; inputs?: RunInputs; log: (...args: any[]) => void}) {
  let pageUrl = '/' + mdFile.replace(/\.md$/, '').replace(/^\//, '').replace(/\\/g, '/')
  if (pageUrl === '/index') pageUrl = '/'
  pageUrl = appendInputsToUrl(pageUrl, inputs)
