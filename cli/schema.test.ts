...
describe('duckdb', () => {
  ...
  it('describes the requested table columns', async () => {
    ...
    expect(output).toContain('carrier string')
    expect(output.trim().endsWith(')')).toBe(true)
  })

  it('normalizes warehouse types for schema output', () => {
    expect(parseWarehouseFieldType('VARCHAR').displayType).toBe('string')
    expect(parseWarehouseFieldType('INTEGER').displayType).toBe('number')
    expect(parseWarehouseFieldType('NUMERIC(10, 2)').displayType).toBe('number')
    expect(parseWarehouseFieldType('VARCHAR[]').displayType).toBe('array<string>')
    expect(parseWarehouseFieldType('INTEGER[]').displayType).toBe('array<number>')
    ...
    expect(parseWarehouseFieldType('Array(String)').displayType).toBe('array<string>')
    expect(parseWarehouseFieldType('ARRAY').displayType).toBe('array')
    expect(formatType(parseWarehouseFieldType('not_a_known_type').type)).toBe('unknown')
  })
})
...
describe.skipIf(!process.env.ATHENA_TEST)('athena', {timeout: 30_000}, () => {
  ...
  it('describes an athena table from the configured database', async () => {
    ...
    expect(output).toContain('table flights (')
    expect(output).toContain('carrier string')
    expect(output).toContain('dep_delay number')
  })
})
...
describe.skipIf(!process.env.SLOW_TEST)('clickhouse', {retry: 3, timeout: 20_000}, () => {
  ...
  it('describes a clickhouse table from the configured database', async () => {
    ...
    expect(output).toContain('pickup_datetime timestamp')
